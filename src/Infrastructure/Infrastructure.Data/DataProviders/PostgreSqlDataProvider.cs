...
using Infrastructure.Data.DataProviders.LinqToDb;
...
public partial class PostgreSqlDataProvider : BaseDataProvider, INopDataProvider
{
    ...
    protected virtual string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : Entity
    {
        ArgumentNullException.ThrowIfNull(dataConnection);

        var descriptor = NopMappingSchema.GetEntityDescriptor(typeof(TEntity))
                         ?? throw new InfrastructureDataException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
        ...
        if (string.IsNullOrEmpty(columnName))
            throw new InfrastructureDataException("A table's primary key does not have an identity constraint");
    ...
    /// </returns>
    public virtual Task<int?> GetTableIdentAsync<TEntity>() where TEntity : Entity
    {
    ...
    public virtual async Task SetTableIdentAsync<TEntity>(int ident) where TEntity : Entity
    {
    ...
    public virtual string BuildConnectionString(INopConnectionStringInfo nopConnectionString)
    {
        ...
        if (nopConnectionString.IntegratedSecurity)
            throw new InfrastructureDataException("Data provider supports connection only with login and password");
